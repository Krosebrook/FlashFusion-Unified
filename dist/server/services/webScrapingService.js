...
class WebScrapingService {
  constructor() {
    ...
    if (process.env.FIRECRAWL_API_KEY && process.env.FIRECRAWL_API_KEY !== 'your_firecrawl_api_key_here') {
      ...
      logger.info('Firecrawl API key not configured - Firecrawl features will be disabled');
    }
  ...
  async extractContent(url, options = {}) {
    try {
      ...
      if (!this.firecrawl) {
        return {
          success: false,
          error: 'Firecrawl not configured - please set FIRECRAWL_API_KEY environment variable'
        };
      }
  ...
  async crawlWebsite(baseUrl, options = {}) {
    try {
      ...
      if (crawlResult.success) {
        logger.info(`Successfully crawled ${crawlResult.data.length} pages from ${baseUrl}`);
        return {
          success: true,
          data: {
            baseUrl,
        ...
        try {
          if (typeof selector === 'string') {
            ...
            if (selector.multiple) {
              extractedData[key] = await Promise.all(elements.map(el => selector.attribute ? el.getAttribute(selector.attribute) : el.textContent()));
            } else if (elements.length > 0) {
              extractedData[key] = selector.attribute ? await elements[0].getAttribute(selector.attribute) : await elements[0].textContent();
            }
          }
        } catch (selectorError) {
          logger.warn(`Failed to extract data for selector '${key}':`, selectorError);
  ...
  async automateForm(url, formData = {}, options = {}) {
    try {
      ...
      if (options.messageSelectors) {
        for (const [type, selector] of Object.entries(options.messageSelectors)) {
          try {
            const element = await page.$(selector);
            if (element) {
      ...
      return {
        ...
        data: {
          ...
          messages,
          screenshots: {
            before: beforeScreenshot ? beforeScreenshot.toString('base64') : null,
  ...
  async monitorPageChanges(url, options = {}) {
    try {
      ...
      const initialHash = require('crypto').createHash('md5').update(initialContent).digest('hex');
      ...
      while (Date.now() < endTime) {
        ...
        try {
          ...
          const currentHash = require('crypto').createHash('md5').update(currentContent).digest('hex');
          if (currentHash !== monitoringData.initialHash && !monitoringData.changes.some(change => change.hash === currentHash)) {
            ...
            logger.info(`Page change detected on ${url} at ${new Date().toISOString()}`);
          }
        } catch (monitorError) {
      ...
      await context.close();
      logger.info(`Page monitoring completed for ${url}. Detected ${monitoringData.changes.length} changes`);
      return {
        success: true,
        data: {
          ...monitoringData,
  ...
  async healthCheck() {
    try {
      ...
      try {
        const testBrowser = await webkit.launch({
          headless: true
        });
        await testBrowser.close();
