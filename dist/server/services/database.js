...
class DatabaseService {
  ...
  async logAgentInteraction(agentId, input, output, modelUsed, userId, metadata = {}) {
    try {
      if (this.supabase) {
        // Use Supabase
        const {
        ...
        const result = await this.query(`INSERT INTO agent_logs (agent_id, input, output, model_used, user_id, metadata)
           VALUES ($1, $2, $3, $4, $5, $6)
           RETURNING *`, [agentId, input.substring(0, 1000), output.substring(0, 2000), modelUsed, userId, metadata]);
    ...
    try {
      if (this.supabase) {
        const {
          count,
          error
        } = await this.supabase.from('agent_logs').select('*', {
          count: 'exact',
          head: true
        });
        if (!error) {
