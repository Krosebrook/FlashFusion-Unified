...
class SecureApiKeyService {
  ...
  static validateApiKey(apiKey) {
    ...
    for (const [providerName, config] of Object.entries(this.KEY_CONFIGS)) {
      if (apiKey.startsWith(config.prefix)) {
        if (apiKey.length >= config.minLength && apiKey.length <= config.maxLength) {
          return {
            isValid: true,
  ...
  static getApiKeyFromEnv(provider) {
    switch (provider.toLowerCase()) {
      ...
      case 'anthropic':
        ...
        return anthropicKey && !anthropicKey.includes('your_') ? anthropicKey : null;
      default:
        return null;
    }
  }
  ...
  static isPlaceholderKey(apiKey) {
    ...
    const placeholderPatterns = ['your_', 'placeholder', 'example', 'test_key', 'demo_key', 'sk-1234567890abcdef'];
    return placeholderPatterns.some(pattern => apiKey.toLowerCase().includes(pattern.toLowerCase()));
  }
}
module.exports = {
  SecureApiKeyService
