 ...
 */
router.post('/extract', [body('url').isURL().withMessage('Valid URL is required'), body('options').optional().isObject()], handleValidationErrors, async (req, res) => {
  try {
    const {
      url,
      options = {}
 ...
 */
router.post('/crawl', [body('baseUrl').isURL().withMessage('Valid base URL is required'), body('options').optional().isObject(), body('options.limit').optional().isInt({
  min: 1,
  max: 1000
}).withMessage('Limit must be between 1 and 1000')], handleValidationErrors, async (req, res) => {
  try {
    const {
      baseUrl,
      options = {}
 ...
 */
router.post('/screenshot', [body('url').isURL().withMessage('Valid URL is required'), body('options').optional().isObject(), body('options.browserType').optional().isIn(['chromium', 'firefox', 'webkit']), body('options.format').optional().isIn(['png', 'jpeg'])], handleValidationErrors, async (req, res) => {
  try {
    const {
      url,
      options = {}
 ...
 */
router.post('/extract-data', [body('url').isURL().withMessage('Valid URL is required'), body('selectors').isObject().withMessage('Selectors object is required'), body('options').optional().isObject()], handleValidationErrors, async (req, res) => {
  try {
    const {
      url,
      selectors,
    ...
    const result = await webScrapingService.extractStructuredData(url, selectors, options);
    if (result.success) {
      res.json(result);
    } else {
      res.status(400).json(result);
 ...
 */
router.post('/automate-form', [body('url').isURL().withMessage('Valid URL is required'), body('formData').isObject().withMessage('Form data object is required'), body('options').optional().isObject()], handleValidationErrors, async (req, res) => {
  try {
    const {
      url,
      formData,
    ...
    const result = await webScrapingService.automateForm(url, formData, options);
    if (result.success) {
      res.json(result);
    } else {
      res.status(400).json(result);
 ...
 */
router.post('/monitor', [body('url').isURL().withMessage('Valid URL is required'), body('options').optional().isObject(), body('options.duration').optional().isInt({
  min: 1000,
  max: 600000
}).withMessage('Duration must be between 1s and 10min'), body('options.interval').optional().isInt({
  min: 1000,
  max: 60000
}).withMessage('Interval must be between 1s and 1min')], handleValidationErrors, async (req, res) => {
  try {
    const {
      url,
      options = {}
 ...
 */
router.post('/batch-extract', [body('urls').isArray({
  min: 1,
  max: 50
}).withMessage('URLs array is required (max 50)'), body('urls.*').isURL().withMessage('All URLs must be valid'), body('options').optional().isObject()], handleValidationErrors, async (req, res) => {
  try {
    const {
      urls,
      options = {}
    ...
    for (const chunk of chunks) {
      const chunkResults = await Promise.all(chunk.map(url => webScrapingService.extractContent(url, options)));
