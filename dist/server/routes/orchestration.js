...
router.post('/projects', async (req, res) => {
  try {
    ...
    const projectId = `ff-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    const request = {
      projectId,
      description,
      priority,
...
router.get('/projects/:projectId', async (req, res) => {
  try {
    ...
    // Get workflow status
    const workflow = await req.orchestrator.workflow.getWorkflowStatus(projectId);
    res.json({
      success: true,
      data: {
        project: context,
...
router.put('/projects/:projectId', async (req, res) => {
  try {
    ...
    const updatedContext = await req.orchestrator.context.updateProjectContext(projectId, updates, 'api_user');
    if (!updatedContext) {
      return res.status(404).json({
        success: false,
        error: 'Project not found'
...
router.post('/agents/analyze', async (req, res) => {
  try {
    ...
    const analysis = await req.orchestrator.roleSelector.analyzeRequest(request);
    const selectedAgents = req.orchestrator.roleSelector.selectOptimalAgents(analysis);
    res.json({
      success: true,
      data: {
        analysis,
...
router.put('/workflows/:projectId/progress', async (req, res) => {
  try {
    ...
    const workflow = await req.orchestrator.workflow.updateProgress(projectId, capability, status, agentRole);
    if (!workflow) {
      return res.status(404).json({
        success: false,
        error: 'Workflow not found'
...
router.post('/communication/messages', async (req, res) => {
  try {
    ...
    const messageId = await req.orchestrator.communication.sendMessage(fromAgent, toAgent, message, priority);
    res.status(201).json({
      success: true,
      data: {
        messageId
