...
const personalizedAgents = {
  writerbot: {
    ...
    basePrompt: "You write emotionally intelligent copy and mental health content.",
...
router.post('/personality-chat', async (req, res) => {
  try {
    ...
    const agent = Object.values(personalizedAgents).find(a => a.task === taskType);
    if (!agent) {
      return res.status(400).json({
        error: `Unknown task type: ${taskType}`,
        availableTasks: [...new Set(Object.values(personalizedAgents).map(a => a.task))]
      });
    }
    ...
    const personalizedPrompt = personalitySystem.getPersonalizedPrompt(agent.personality, `${agent.basePrompt}\n\nUser request: ${input}`, fullContext);

    // Log personality state
    logger.info(`Agent ${agent.personality} responding with mood: ${personalitySystem.getEmotionalState(agent.personality, fullContext)}`);

    // Here you would call your AI service with the personalized prompt
    // For now, we'll simulate a response
    const simulatedResponse = `I understand you need help with: "${input}"\n\nAs a ${agent.personality}, I'll approach this with my ${personalitySystem.personalities[agent.personality].decision_bias} perspective.`;

    // Generate personalized response
    const finalResponse = personalitySystem.generateAgentResponse(agent.personality, simulatedResponse, fullContext);
    ...
    res.json({
      ...
      context: {
        emotionalState: personalitySystem.getEmotionalState(agent.personality, fullContext),
...
router.post('/check-conflicts', async (req, res) => {
  try {
    ...
    const strategy = personalitySystem.suggestCollaborationStrategy(agent1, agent2);
    res.json({
      success: true,
  ...
  try {
    ...
    for (const [agentId, agent] of Object.entries(personalizedAgents)) {
      ...
      agentStates[agentId] = {
        ...
        emotionalState: personalitySystem.getEmotionalState(agent.personality, context),
        workload: context.workload,
        lastInteraction: context.lastInteraction,
        communicationStyle: personalitySystem.personalities[agent.personality].communication_style
      };
    }
    res.json({
      success: true,
  ...
  try {
    ...
    for (let i = 0; i < teamMembers.length; i++) {
      for (let j = i + 1; j < teamMembers.length; j++) {
        ...
        const conflict = personalitySystem.detectPersonalityClashes(member1, member2);
        const strategy = personalitySystem.suggestCollaborationStrategy(member1, member2);
        if (conflict) {
          conflicts.push({
    ...
    res.json({
      ...
      conflicts,
      collaborations,
      recommendation: conflicts.length > 0 ? 'Consider involving a mediator for conflict resolution' : 'Team has good natural dynamics'
    });
  } catch (error) {
...
router.get('/recommend-agent/:task', async (req, res) => {
  try {
    ...
    const agent = Object.entries(personalizedAgents).find(([_, config]) => config.personality === personality);
    if (!agent) {
      return res.status(404).json({
