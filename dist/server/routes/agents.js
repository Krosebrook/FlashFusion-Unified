...
const agents = {
  writerbot: {
    ...
    system: "You write emotionally intelligent copy and mental health content. Keep tone grounded, human, and psychologically aware.",
  ...
  coderbot: {
    model: "openai/gpt-4o",
    system: "You are a full-stack developer skilled in Firebase, Replit, and prompt-based apps. Respond with valid, tested code.",
  ...
  logicbot: {
    ...
    system: "You design agent workflows and automation logic using JSON, YAML, and prompt chaining.",
  ...
  securitybot: {
    model: "anthropic/claude-3-opus",
    system: "You are a cybersecurity expert focused on defensive security, vulnerability analysis, and secure coding practices. Never provide malicious code.",
    fallback: "openai/gpt-4o",
...
async function setAgentMemory(agentId, memory) {
  try {
    ...
    if (redisClient) {
      await redisClient.setEx(`agent_memory:${agentId}`, 3600, JSON.stringify(trimmedMemory)); // 1 hour TTL
    } else {
...
async function logToSupabase(agentId, input, output, model, userId, metadata = {}) {
  try {
...
router.post('/chat', agentLimiter, async (req, res) => {
  try {
    ...
    if (!agent) {
      return res.status(400).json({
        error: `Unknown taskType: ${taskType}. Available: ${Object.values(agents).map(a => a.task).join(', ')}`
      });
    }
    ...
    try {
      ...
      if (!reply || response.status !== 200) {
        throw new Error(`Primary model failed: ${data?.error?.message || 'Unknown error'}`);
      }
    } catch (primaryError) {
      logger.warn(`Primary model ${agent.model} failed, trying fallback: ${primaryError.message}`);
      ...
      try {
        ...
        if (!reply || fallbackResponse.status !== 200) {
          throw new Error(`Fallback model failed: ${fallbackData?.error?.message || 'Unknown error'}`);
        }
      } catch (fallbackError) {
        logger.error(`Both models failed for agent ${agent.task}:`, fallbackError);
