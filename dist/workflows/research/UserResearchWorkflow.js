...
class UserResearchWorkflow {
  ...
  async initializeResearcherAgent(workflowId) {
    return {
      ...
      capabilities: ['interview_script_generation', 'participant_recruitment', 'interview_analysis', 'survey_design', 'behavioral_data_collection'],
      tools: {
  ...
  async initializeAnalystAgent(workflowId) {
    return {
      ...
      capabilities: ['statistical_analysis', 'pattern_recognition', 'behavioral_data_analysis', 'metric_calculation', 'trend_identification'],
      tools: {
  ...
  async initializeInsightSynthesizer(workflowId) {
    return {
      ...
      capabilities: ['qualitative_coding', 'theme_identification', 'persona_generation', 'journey_mapping', 'recommendation_creation'],
      tools: {
        coding: 'atlas_ti_api',
  ...
  async executeAnalysisPhase(workflow, agents, studyConfig) {
  ...
  getResearchQuestions(studyType) {
    const questions = {
      'ai_developer_discovery': ['What are the biggest time sinks in AI development workflows?', 'Where do developers get stuck when integrating multiple AI services?', 'What would make AI orchestration 10x easier?'],
      'ecommerce_automation': ['What manual tasks consume the most time in multi-platform selling?', 'Where do current automation tools fail?', 'What would perfect e-commerce automation look like?'],
      'content_optimization': ['How do creators currently adapt content for different platforms?', 'What are the biggest bottlenecks in content repurposing?', 'What would ideal content multiplication workflow include?']
    };
