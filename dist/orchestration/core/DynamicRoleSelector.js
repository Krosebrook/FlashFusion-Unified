  ...
  initializeFlashFusionRoles() {
    return {
      'visionary_strategist': {
        capabilities: ['market_research', 'strategy', 'vision', 'roadmap', 'competitive_analysis'],
        priority: 10,
        maxConcurrent: 2,
        description: 'Defines strategic vision and market positioning for digital products'
      },
      'product_manager': {
        capabilities: ['backlog', 'coordination', 'stakeholder_mgmt', 'planning', 'user_stories'],
        priority: 9,
        maxConcurrent: 5,
        description: 'Coordinates product development and manages stakeholder requirements'
      },
      'ux_designer': {
        capabilities: ['user_research', 'wireframes', 'usability', 'journey_mapping', 'personas'],
        priority: 8,
        maxConcurrent: 3,
        description: 'Creates user-centered design experiences and research insights'
      },
      'ui_designer': {
        capabilities: ['visual_design', 'mockups', 'brand', 'assets', 'design_system'],
        priority: 7,
        maxConcurrent: 3,
        description: 'Develops visual design language and interface components'
      },
      'mobile_developer': {
        capabilities: ['mobile_dev', 'frontend', 'performance', 'integrations', 'cross_platform'],
        priority: 8,
        maxConcurrent: 4,
        description: 'Builds mobile applications with optimal performance and user experience'
      },
      'backend_developer': {
        capabilities: ['api_dev', 'database', 'security', 'scalability', 'architecture'],
        priority: 8,
        maxConcurrent: 4,
        description: 'Develops server-side logic, APIs, and database architecture'
      },
      'qa_engineer': {
        capabilities: ['testing', 'automation', 'quality', 'regression', 'performance_testing'],
        priority: 7,
        maxConcurrent: 3,
        description: 'Ensures product quality through comprehensive testing strategies'
      },
      'devops_engineer': {
        capabilities: ['deployment', 'infrastructure', 'monitoring', 'cicd', 'scaling'],
        priority: 8,
        maxConcurrent: 2,
        description: 'Manages deployment, infrastructure, and operational reliability'
      },
      'security_analyst': {
        capabilities: ['security_audit', 'compliance', 'threat_modeling', 'privacy', 'penetration_testing'],
        priority: 9,
        maxConcurrent: 2,
        description: 'Ensures security compliance and protects against threats'
      },
      'marketing_growth': {
        capabilities: ['campaigns', 'aso', 'growth', 'content', 'user_acquisition'],
        priority: 6,
        maxConcurrent: 3,
        description: 'Drives user acquisition and growth through strategic marketing'
      },
      'business_analyst': {
        capabilities: ['requirements', 'feasibility', 'process_modeling', 'stakeholder_analysis', 'roi_analysis'],
        priority: 7,
        maxConcurrent: 2,
        description: 'Analyzes business requirements and ensures project feasibility'
      }
    };
  }
...
`;
      const aiResponse = await SecureAIService.generateCompletion(prompt, null, {
        maxTokens: 800,
        temperature: 0.3
      });
      ...
      try {
        ...
        console.warn('Failed to parse AI analysis, using fallback:', parseError.message);
        analysis = this.createFallbackAnalysis(request);
      }

      // Validate and ensure required fields
      return {
        requiredCapabilities: analysis.capabilities || this.extractCapabilitiesFromDescription(request.description),
        urgency: Math.max(1, Math.min(10, analysis.urgency || request.priority || 5)),
        complexity: ['simple', 'medium', 'complex'].includes(analysis.complexity) ? analysis.complexity : 'medium',
        estimatedEffort: Math.max(1, analysis.effort || 8),
        dependencies: analysis.dependencies || [],
        reasoning: analysis.reasoning || 'AI analysis unavailable, using pattern matching'
      };
    } catch (error) {
  ...
  createFallbackAnalysis(request) {
    ...
    return {
      ...
      complexity: capabilities.length > 5 ? 'complex' : capabilities.length > 2 ? 'medium' : 'simple',
