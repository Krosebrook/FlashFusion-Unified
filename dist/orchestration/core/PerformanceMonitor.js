...
class PerformanceMonitor extends EventEmitter {
  ...
  async initialize() {
    try {
      await fs.mkdir(this.metricsPath, {
        recursive: true
      });
  ...
  async checkThresholds(metricName, value, metadata) {
    ...
    if (alertLevel) {
      ...
      console.warn(`🚨 ${alertLevel.toUpperCase()} Alert: ${metricName} = ${value} (threshold: ${threshold[alertLevel]})`);
    }
  }
  isCriticalMetric(metricName) {
    const criticalMetrics = ['agent_error', 'handoff_timeout', 'ai_service_failure', 'system_error', 'memory_heap_used', 'response_time'];
  ...
  async recordAIServicePerformance(provider, model, tokens, duration, success = true) {
  ...
  async getMetrics(metricName, timeRange = 3600000) {
    ...
    const filtered = Array.from(this.metrics.values()).filter(metric => metric.name === metricName && metric.timestamp >= cutoff).sort((a, b) => a.timestamp - b.timestamp);
    return filtered;
  }
  ...
  async getDashboardData() {
    ...
    const dashboard = {
      ...
      handoffs: {
        performance: await this.getAggregatedMetrics('handoff_performance', hour),
        failures: await this.getAggregatedMetrics('handoff_failure', hour)
      },
      ai: {
        performance: await this.getAggregatedMetrics('ai_service_performance', hour),
      ...
      alerts: {
        active: Array.from(this.alerts.values()).filter(alert => alert.status === 'active'),
        recent: Array.from(this.alerts.values()).filter(alert => now - alert.timestamp < hour).sort((a, b) => b.timestamp - a.timestamp).slice(0, 10)
      },
      timestamp: now
    };
    return dashboard;
  ...
  async getActiveAlerts() {
    return Array.from(this.alerts.values()).filter(alert => alert.status === 'active').sort((a, b) => b.timestamp - a.timestamp);
  }
