...
class WorkflowStateManager extends EventEmitter {
  ...
  async initialize() {
    try {
      await fs.mkdir(this.workflowPath, {
        recursive: true
      });
  ...
  async getWorkflow(projectId) {
    ...
    if (!workflow) {
      try {
        ...
        console.warn(`Workflow not found for project ${projectId}:`, error.message);
      }
    }
    return workflow;
  }
  async updateProgress(projectId, capability, status, agentRole = null) {
    ...
    console.log(`📊 Progress updated - ${projectId}: ${capability} = ${status}`);
    return workflow;
  }
  async checkPhaseCompletion(workflow, phase) {
    const phaseCapabilities = workflow.phases[phase];
    const completedCount = Object.values(phaseCapabilities).filter(status => status === 'completed').length;
    ...
    if (phaseProgress >= 80) {
      ...
      if (currentIndex >= 0 && currentIndex < phases.length - 1) {
        ...
        if (currentIndex >= currentPhaseIndex) {
          ...
          console.log(`🎯 Phase advanced - ${workflow.projectId}: ${phase} → ${nextPhase}`);
        }
      }
    }
  }
  calculatePhaseProgress(phases) {
    ...
    for (const [phase, capabilities] of Object.entries(phases)) {
      ...
      const completed = Object.values(capabilities).filter(status => status === 'completed').length;
  ...
  calculateOverallProgress(phases) {
    ...
    const completed = allCapabilities.filter(status => status === 'completed').length;
  ...
  determineWorkflowStatus(workflow) {
    ...
    if (overall < 25) return 'planning';
    if (overall < 50) return 'development';
    if (overall < 75) return 'testing';
    if (overall < 100) return 'deployment';
    return 'completed';
  }
  ...
  async getAnalytics() {
    ...
    for (const workflow of this.activeWorkflows.values()) {
      ...
      for (const [phase, progress] of Object.entries(workflow.progress.byPhase)) {
    ...
    analytics.averageProgress = this.activeWorkflows.size > 0 ? Math.round(totalProgress / this.activeWorkflows.size) : 0;
