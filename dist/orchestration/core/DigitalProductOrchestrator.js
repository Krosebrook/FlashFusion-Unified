...
const {
  SecureApiKeyService
} = require('../../server/services/apiKeyService');
...
class DigitalProductOrchestrator extends EventEmitter {
  ...
  async initialize() {
    ...
    try {
      ...
      console.log('🎭 Digital Product Orchestrator initialized for FlashFusion');
      return {
        status: 'initialized',
        services: {
          ai: aiHealth.status,
  ...
  async processProductRequest(request) {
    try {
      ...
      console.log(`📋 Selected ${selectedAgents.length} agents:`, selectedAgents.map(a => a.role));
      ...
      const result = await this.executeFlashFusionWorkflow(selectedAgents, request, context);
      return {
        success: true,
  ...
  async executeFlashFusionWorkflow(selectedAgents, request, context) {
    const results = [];
    for (const agent of selectedAgents) {
      try {
        ...
        const agentContext = await this.context.getRelevantContext(request.projectId, agent.role);

        // Build agent prompt
        const prompt = this.buildFlashFusionAgentPrompt(agent, request, agentContext);
        ...
        await this.context.updateProjectContext(request.projectId, {
          [agent.capability]: agentResult
        }, agent.role);

        // Update progress
        await this.workflow.updateProgress(request.projectId, agent.capability, 'completed');
        results.push({
          agent: agent.role,
...
- Risk Assessment Matrix`
    ...
    return rolePrompts[agent.role] || `You are a ${agent.role.replace('_', ' ')} for FlashFusion. Handle this request: ${request.description}`;
  }
  async getDashboard() {
    const aiHealth = await SecureAIService.healthCheck();
    return {
  ...
  async getActiveWorkflows() {
    const workflows = [];
    for (const [projectId, workflow] of this.workflow.activeWorkflows) {
      try {
  ...
  getAgentStatus() {
    ...
    return {
      ...
      // All agents are AI-powered, so always available
