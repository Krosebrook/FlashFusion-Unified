...
class ContextManager {
  ...
  async initialize() {
    try {
      ...
      await fs.mkdir(this.contextPath, {
        recursive: true
      });
  ...
  async getProjectContext(projectId) {
    ...
    if (!context) {
      try {
        ...
        console.warn(`Context not found for project ${projectId}:`, error.message);
      }
    }
    return context;
  }
  ...
  async getContextSummary() {
    return {
      ...
      activeProjects: Array.from(this.projectContexts.values()).filter(ctx => ctx.project.status === 'active').length,
