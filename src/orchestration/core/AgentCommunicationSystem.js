...
class AgentCommunicationSystem extends EventEmitter {
  ...
  async sendMessage(fromAgent, toAgent, message, priority = 'normal') {
    ...
    if (this.redis) {
      try {
        await this.redis.setEx(`ff:message:${messageId}`, 3600, JSON.stringify(messageData));
        await this.redis.publish('ff:agent:messages', JSON.stringify({
          type: 'new_message',
          agent: toAgent,
          messageId
        }));
  ...
  async initiateHandoff(fromAgent, toAgent, deliverables, timeout = 300000) {
    ...
    if (this.redis) {
      try {
        await this.redis.setEx(`ff:handoff:${handoffId}`, 3600, JSON.stringify(handoffData));
        await this.redis.publish('ff:agent:handoffs', JSON.stringify({
  ...
  async completeHandoff(handoffId, deliverables) {
    ...
    if (this.redis) {
      try {
        await this.redis.setEx(`ff:handoff:${handoffId}`, 86400, JSON.stringify(handoffData));
      } catch (error) {
