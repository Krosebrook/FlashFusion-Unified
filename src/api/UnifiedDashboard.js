...
class UnifiedDashboard {
    ...
    try {
      ...
      const health = {
        ...
        services: {
          database: dbHealth,
          ai: aiHealth,
          agentOrchestrator: this.agentOrchestrator ? this.agentOrchestrator.getHealth() : { status: 'inactive' },
          workflowEngine: this.workflowEngine ? this.workflowEngine.getHealth() : { status: 'inactive' }
        }
      };
  ...
  async getSystemStatus(req, res) {
    try {
      ...
      const agentPersonalities = personalitiesResult.success ? personalitiesResult.data : [];
      
      // Get workflow data
      const workflowStatus = this.workflowEngine ? this.workflowEngine.getAllWorkflows() : { data: { workflows: [] } };
  ...
  async getAgent(req, res) {
    try {
      ...
      const agent = agentStatus.data.agents.roles.find(a => a.id === id);
