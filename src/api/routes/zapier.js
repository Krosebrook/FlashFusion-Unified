...
router.post('/webhooks/register', async (req, res) => {
    try {
        ...
        const webhook = zapierService.registerWebhook(eventType, webhookUrl, config);
...
router.post('/webhooks/trigger', async (req, res) => {
    try {
        ...
        const result = await zapierService.triggerWebhook(eventType, data || {}, options || {});
        res.json(result);
    } catch (error) {
...
router.post('/workflows/automation', async (req, res) => {
    try {
        ...
        const result = await zapierService.createWorkflowAutomation(workflowId, zapierConfig);
        res.json(result);
    } catch (error) {
...
router.post('/webhooks/chain', async (req, res) => {
    try {
        ...
        const result = await zapierService.chainWebhooks(eventType, data || {}, webhookUrls);
        res.json(result);
    } catch (error) {
...
router.post('/incoming-webhook', async (req, res) => {
    try {
        ...
        const result = await zapierService.processIncomingWebhook(webhookData, headers);
    ...
    try {
        ...
        switch (automationType) {
            case 'workflow_completed':
                testResults.workflow = await zapierService.triggerWebhook('workflow_completed', {
            ...
            case 'lead_generated':
                testResults.lead = await zapierService.triggerWebhook('lead_generated', {
                    leadId: 'lead-001',
            ...
            case 'agent_action':
                testResults.agent = await zapierService.triggerWebhook('agent_action_completed', {
