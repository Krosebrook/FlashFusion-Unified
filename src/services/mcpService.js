...
class MCPService extends EventEmitter {
    ...
    async createServerInstances() {
        ...
        this.logger.info('SERVICE', `Created ${this.servers.size} server instances`);
    }
    ...
    async autoStartServers() {
        const autoStartServers = Array.from(this.servers.entries())
            ...
            .sort(([_, a], [__, b]) => this.getPriorityValue(b.config.priority) - this.getPriorityValue(a.config.priority));
        ...
        for (const [name, server] of autoStartServers) {
            try {
                ...
                this.logger.error('SERVICE', `Failed to auto-start ${name}`, { error: error.message });
            }
        }
    }
    ...
    async installDependencies() {
        const typescriptServers = Array.from(this.servers.entries())
            ...
            .map(([name, server]) => ({ name, path: require('path').join(this.config.basePath, server.config.path) }));
        ...
        const installPromises = typescriptServers.map(async (server) => {
            try {
                ...
                this.logger.warn('SETUP', `Failed to install dependencies for ${server.name}`, { error: error.message });
            }
        });
    ...
    async startServer(serverName, options = {}) {
        try {
            ...
            this.logger.error('SERVICE', `Failed to start ${serverName}`, { error: error.message });
            throw error;
        }
    }
    ...
    async stopServer(serverName, force = false) {
        try {
            ...
            this.logger.error('SERVICE', `Failed to stop ${serverName}`, { error: error.message });
            throw error;
        }
    }
    ...
    async stopAllServers(force = false) {
        ...
        const stopPromises = Array.from(this.servers.keys()).map(name => 
            this.stopServer(name, force).catch(error => 
                this.logger.error('SERVICE', `Failed to stop ${name}`, { error: error.message })
            )
        );
