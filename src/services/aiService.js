    ...
    async generateAgentResponse(agentId, message, context = {}) {
        ...
        try {
            ...
            if (personalityResult.success) {
                agentPersonality = personalityResult.data.find(p => p.agent_id === agentId);
            }
    ...
    buildSystemPrompt(agentId, personality) {
        const defaultPersonalities = {
            ...
            'automator': {
                ...
                traits: ['efficient', 'systematic', 'technology-focused', 'solution-oriented'],
            ...
            'optimizer': {
                ...
                traits: ['performance-focused', 'improvement-oriented', 'strategic', 'results-driven'],
                style: 'Goal-oriented and metrics-focused',
                signature: 'Optimizing for excellence! 🚀'
            }
        };

        const agentInfo = personality || defaultPersonalities[agentId] || defaultPersonalities['coordinator'];
        
        let systemPrompt = `You are ${agentInfo.name || agentId}, a specialized AI agent in the FlashFusion platform.

Your Role: ${agentInfo.role || 'General assistance and coordination'}

Your Personality Traits: ${JSON.stringify(agentInfo.traits || ['helpful', 'professional'])}
...
Context: You are part of FlashFusion, an AI Business Operating System that helps turn ideas into automated revenue streams. Work collaboratively and maintain a professional yet personable tone.
...
4. End with your signature phrase`;
        ...
        if (personality) {
            if (personality.communication_style) {
                systemPrompt += `\n\nCommunication Preferences: ${JSON.stringify(personality.communication_style)}`;
            }
    ...
    async saveConversationToMemory(agentId, userId, conversationId, userMessage, agentResponse, sequenceStart) {
        try {
            // Save user message
    ...
    async getAgentStats(agentId, days = 7) {
        try {
            ...
            const avgResponseTime = recentLogs.reduce((sum, log) => 
                sum + (log.metadata?.response_time_ms || 0), 0
            ) / totalInteractions || 0;

            const totalTokens = recentLogs.reduce((sum, log) => 
                sum + (log.metadata?.tokens_used || 0), 0
            );
