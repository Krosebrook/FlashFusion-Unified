...
class NotionService {
    ...
    async search(query, filters = {}) {
        ...
        try {
            ...
            const results = response.results.map(item => ({
                id: item.id,
                object: item.object,
                title: item.object === 'page' ? this.extractPageTitle(item) : 
        ...
        Object.entries(properties).forEach(([key, prop]) => {
            switch (prop.type) {
                case 'title':
                    ...
                    break;
                case 'rich_text':
                    extracted[key] = prop.rich_text?.map(t => t.plain_text).join('') || '';
                    break;
                case 'number':
    ...
    getHealth() {
        return {
            service: 'notion',
            status: this.isInitialized && this.connectionStatus.connected ? 'healthy' : 'unhealthy',
            initialized: this.isInitialized,
