...
class MCPServerInstance extends EventEmitter {
    ...
    async start(options = {}) {
        ...
        try {
            ...
            const serverPath = require('path').join(this.config.basePath, this.config.path);
    ...
    async sendRequest(method, params = {}, timeout = 30000) {
        if (this.status !== 'running') {
            throw new Error(`Server ${this.name} is not running (status: ${this.status})`);
        }
    ...
    setupProcessHandlers() {
        ...
        if (this.process.stderr) {
            this.process.stderr.on('data', (data) => {
                this.logger.warn('SERVER', `${this.name} stderr`, { output: data.toString() });
            });
        }
    }
        ...
        for (const line of lines) {
            try {
                ...
                if (response.id && this.requestQueue.has(response.id)) {
                    const { resolve, reject, timeoutHandle } = this.requestQueue.get(response.id);
                    clearTimeout(timeoutHandle);
                ...
                this.logger.debug('SERVER', `Non-JSON output from ${this.name}`, { output: line });
            }
        }
    }
