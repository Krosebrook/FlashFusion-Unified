...
router.post('/screenshot',
    [
        body('url').isURL().withMessage('Valid URL is required'),
        body('options').optional().isObject(),
        body('options.browserType').optional().isIn(['chromium', 'firefox', 'webkit']),
...
router.post('/monitor',
    [
        body('url').isURL().withMessage('Valid URL is required'),
        body('options').optional().isObject(),
        body('options.duration').optional().isInt({ min: 1000, max: 600000 }).withMessage('Duration must be between 1s and 10min'),
        body('options.interval').optional().isInt({ min: 1000, max: 60000 }).withMessage('Interval must be between 1s and 1min')
    ],
    handleValidationErrors,
    async (req, res) => {
        try {
...
router.post('/batch-extract',
    [
        body('urls').isArray({ min: 1, max: 50 }).withMessage('URLs array is required (max 50)'),
