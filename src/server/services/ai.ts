...
export class SecureAIService {
  private static readonly SUPPORTED_PROVIDERS = ['openai', 'anthropic'] as const;
   ...
   */
  private static createClientForProvider(provider: 'openai' | 'anthropic', apiKey: string) {
    switch (provider) {
      case 'openai':
   ...
   */
  public static getProviderFromApiKey(apiKey: string): 'openai' | 'anthropic' | 'unknown' {
    const validation = SecureApiKeyService.validateApiKey(apiKey);
  ...
  public static async generateCompletion(
    ...
    try {
      ...
      const provider = this.getProviderFromApiKey(userApiKey || this.getDefaultApiKey() || '');
  ...
  public static async healthCheck(): Promise<{
    ...
    for (const provider of this.SUPPORTED_PROVIDERS) {
      try {
        ...
        results.providers[provider] = false;
        results.errors.push(`${provider}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }

    // Determine overall status
    const healthyProviders = Object.values(results.providers).filter(Boolean).length;
