...
export class MCPWrapper {
  ...
  private readonly auditPath: string = 'C:\\Users\\kyler\\FlashFusion\\mcp_audit\\logbook.json';
  private readonly backupPath: string = 'C:\\Users\\kyler\\FlashFusion\\mcp_sandbox\\backups';
  ...
  async executeMutation(
    ...
    try {
      ...
      await fs.promises.writeFile(mutation.file_path, mutation.new_content || '');
  ...
  async rollback(mutation: MCPMutation): Promise<void> {
    if (mutation.backup_path && mutation.original_content !== undefined) {
      await fs.promises.writeFile(mutation.file_path, mutation.original_content);
  ...
  private async saveAuditLog(): Promise<void> {
    const logData = JSON.parse(await fs.promises.readFile(this.auditPath, 'utf-8'));
    logData.audit_entries = this.auditLog;
    await fs.promises.writeFile(this.auditPath, JSON.stringify(logData, null, 2));
  }
