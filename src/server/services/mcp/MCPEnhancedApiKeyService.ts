...
import { SecureApiKeyService, ApiKeyValidation, ApiKeyConfig } from '../apiKeyService';
...
export class MCPEnhancedApiKeyService extends SecureApiKeyService {
  ...
  static async addProviderWithGovernance(
    ...
    ${providerName}: {
      prefix: process.env.${providerName.toUpperCase()}_KEY_PREFIX || '${config.prefix}',
...
This change requires human approval. Use MCPWrapper.approveCheckpoint('${checkpoint.id}') to proceed.
==========================================
    `);
