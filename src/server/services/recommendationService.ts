...
export class RecommendationService {
  ...
  static async getRecommendations(userId: string): Promise<Recommendation[]> {
    try {
      ...
      const personalizedRecs = await this.getPersonalizedRecommendations(userId);
   ...
   */
  private static async getPersonalizedRecommendations(userId: string): Promise<Recommendation[]> {
    try {
      ...
      const prompt = `Generate 5 content recommendations for a user with these preferences: ${JSON.stringify(userPrefs)}`;
      const aiResponse = await SecureAIService.generateCompletion(prompt, undefined, {
        maxTokens: 500,
        temperature: 0.7
      });
  ...
  private static getDefaultContent(): Recommendation[] {
    return [
      { 
        ...
        type: 'guide',
        description: 'Learn the basics of our platform and get up to speed quickly.'
      },
      { 
        id: 'default2', 
  ...
  private static getEmergencyContent(): Recommendation[] {
    return [{
      ...
      type: 'placeholder',
      description: 'We\'re experiencing some issues. Please try again in a moment.'
    }];
  }
   ...
   */
  static async getTrendingContent(limit: number = 5): Promise<Recommendation[]> {
    try {
   ...
   */
  static async cacheRecommendations(userId: string, recommendations: Recommendation[]): Promise<void> {
    try {
      ...
      if (typeof globalThis !== 'undefined') {
        (globalThis as any).recommendationCache = (globalThis as any).recommendationCache || {};
   ...
   */
  static async getCachedRecommendations(userId: string): Promise<Recommendation[] | null> {
    try {
      const cacheKey = `recommendations_${userId}`;
      
      if (typeof globalThis !== 'undefined' && (globalThis as any).recommendationCache) {
